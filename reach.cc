...
void DF_Inv(float* xdot,float t,float* x,float* DR,float alpha)
{
...
// external force - perturbation
    ...
    float Rx=L2*sin(x[1]),Ry=L2*cos(x[1]);
    float X=L1*sin(x[0])+Rx,Y=L1*cos(x[0])+Ry;
    float Xdot=L1*cos(x[0])*x[2]+L2*cos(x[1])*x[3],Ydot=-L1*sin(x[0])*x[2]+-L2*sin(x[1])*x[3];
    float Fx=-alpha*Ydot, Fy=alpha*Xdot;
    float q2f=Rx*Fy-Ry*Fx;
    float q1f=X*Fy-Y*Fx;

//    %---- calculating total torques in upper and lower joints;

	float Q1 = f1c-f1g+q1+q1f; 
	float Q2 = f2c-f2g+q2+q2f;
...
void cbnet(float *DR,float wcb[][6])
{
	float cb[6]={};
	for(int i=0;i<6;i++) 
        for(int j=0;j<6;j++) 
            cb[i]+=wcb[i][j]*DR[j];  // sum oveer all j
...
void reach(float* x,float* y,float alpha,float wcb[][6])
{
    ...
    for(int i=0;i<n;i++)
	{
        ...
        //if(wcb != NULL)
        { 
            cbnet(DR,wcb);  // affect dynamic drives only

            // now add static drives
            for(int k=0;k<6;k++) 
            { 
                DR[k]+=DR_Al_Di[0][0][k];
            }
...
int main(int argc,char** argv)
{
	...
	for(int i=1;i<argc;i++)
	{
		...
		else if(strcmp(argv[i],"-i")==0) ifstream("ini")>>x[0]>>x[1];
